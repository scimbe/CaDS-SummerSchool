...
def aufbereiten(rohdaten: dict, name: str, schadstoffe: list[str]) -> dict:
    ...
    werte = []
    for schlüssel in schadstoffe:
        wert = rohdaten["messwerte"].get(schlüssel)
        if wert is None:
            continue
        ...
        werte.append({
            ...
            "name": SCHADSTOFFE[schlüssel]["name"],
            "einheit": SCHADSTOFFE[schlüssel]["einheit"],
            "wert": wert,
            "stufe": stufe,
            ...
            "balken": min(100, round(wert / (
                grenzwert if bezug == "grenzwert" and grenzwert else SCHWELLEN[schlüssel][-1]
            ) * 100)),
...
def sortieren(zeilen: list[dict], nach: str) -> list[dict]:
    ...
    return sorted(gültig, key=schlüssel) + fehler
...
def kennzahlen(zeilen: list[dict]) -> dict:
    ...
    alle_werte = [w for z in gültig for w in z["werte"]]
    mittel = {}
    for schlüssel in SCHADSTOFFE:
        passend = [w["wert"] for w in alle_werte if w["schluessel"] == schlüssel]
        if passend:
            mittel[schlüssel] = round(sum(passend) / len(passend), 1)

    return {
        "anzahl": len(gültig),
        "fehlend": [z["slug"] for z in zeilen if not z["ok"]],
        "hoechste": gültig[0]["name"] if gültig else None,
        "mittelwerte": mittel,
        "ueber_eu": [
            {"stadt": z["name"], "schadstoffe": [w["name"] for w in z["werte"] if w["ueber_eu"]]}
            for z in gültig if any(w["ueber_eu"] for w in z["werte"])
        ],
        "stufen_verteilung": {
            stufe: sum(1 for z in gültig if z["stufe"] == stufe)
            for stufe in INDEX_STUFEN
            if any(z["stufe"] == stufe for z in gültig)
        },
    }
